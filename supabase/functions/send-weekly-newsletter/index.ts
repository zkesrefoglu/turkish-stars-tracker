...
const createNewsletterHTML = (articles: NewsArticle[], weekStart: string, weekEnd: string) => {
                ...
                <tr>
                  <td>
                    ...
                    <p style="color: #333; font-size: 16px; line-height: 26px; margin: 0 0 32px 0;">
                      Here are the top stories from this week. Stay informed with the latest updates from around the world.
                    </p>
...
serve(async (req) => {
  ...
  try {
    ...
    // Get published articles from the last 7 days
    ...
    if (!articles || articles.length === 0) {
      ...
      return new Response(
        JSON.stringify({ message: "No articles to send this week" }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    const html = createNewsletterHTML(articles as NewsArticle[], weekStart, weekEnd);
